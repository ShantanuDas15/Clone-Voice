...
| C2 — No loadable model checkpoints | ⚠️ Partial (2/3 milestones) | `f253d3d`, `9a2d8c7` | **C2.1 done** (`9a2d8c7`): real Tacotron2/WaveRNN architecture vendored (`backend/services/sv2tts/`, MIT, provenance in `THIRD_PARTY_NOTICE.md`), `load_models()` verifies each checkpoint's SHA256 against a pinned manifest (`backend/weights_manifest.json`) before `torch.load` ever touches the file, then loads the real `state_dict`. Real checkpoints (`https://huggingface.co/CorentinJ/SV2TTS`, MIT) were downloaded and verified this session; a full `load_models() → synthesize_speech() → vocode()` run through the actual production code path produced a valid waveform (opt-in test `test_real_weights_end_to_end_synthesis`, run manually — not part of the default suite per CLAUDE.md §4.2). H1/H2 were folded in since real weights cannot run without them. Validated: 136/136 pass + 1 opt-in skipped, zero errors. **Still open — C2.2:** scripted, resumable `--fetch` download via `huggingface_hub` (operators currently provision the two files manually, per README.md § Model Weights). **Still open — C2.3:** `/health` `checksum_verified` flag and a post-deploy CLI smoke-test. |
...
- **C2:** ⚠️ Partial (`f253d3d`, `9a2d8c7`) — Placeholder-writing removed, real vendored architecture loads real checksum-verified checkpoints (C2.1 done). **Still open:** scripted `--fetch` download tooling (C2.2), `/health` checksum flag (C2.3) — see Task Status Log.
...
2. **C2.2/C2.3 scope (new):** `--fetch` download tooling and the `/health` checksum flag are deliberately deferred to keep C2.1 reviewable and mergeable on its own — see Task Status Log. Needs a decision on whether `huggingface_hub` is an acceptable new runtime dependency (vs. hand-rolled HTTPS streaming) before C2.2 starts.
