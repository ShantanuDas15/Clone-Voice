...
| **B. ML/Inference** | GPU Memory Management | ✅ Fixed | **High** | `services/tts_pipeline.py` — `_free_gpu_memory()`, `synthesize_speech()`, `vocode()`, `embed_speaker()` · commit `8c85fed` | Resolved: `_free_gpu_memory()` calls `torch.cuda.empty_cache()` after each forward pass (no-op on CPU); intermediate tensors are explicitly `del`eted first so the caching allocator can reclaim them promptly. |
