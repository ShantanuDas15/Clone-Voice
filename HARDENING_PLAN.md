...
| C2 — No loadable model checkpoints | ⚠️ Partial | `f253d3d` | Scoped to fail-fast/reporting per user direction — not vendoring a real pretrained model (separate multi-session effort: architecture porting + licensing review). `download_weights.py` no longer writes placeholder `tacotron.pt`/`wavernn.pt` bytes that masked missing weights from `Path.exists()` checks; it now only warms the real `VoiceEncoder` cache and reports (log + non-zero exit) whether `synthesizer.pt`/`vocoder.pt` are present, without validating them. `load_models()`'s existing fail-loudly `RuntimeError` behavior (already true pre-fix) is now backed by an end-to-end test against a genuinely empty `WEIGHTS_DIR`, not just a mocked `torch.jit.load`. Confirmed no `WEIGHTS_DIR` override in `.env` and network access available — no real checkpoint exists anywhere reachable. Validated: 10/10 new tests pass, live run of `python -m backend.download_weights` exits 1 with a clear per-file message and creates zero files. Full suite 113/113 pass, zero errors. **Remains open:** the repo still has no real synthesizer/vocoder checkpoint to run inference — see §4 item 1, now answered "no," and README.md § Model Weights for the manual provisioning steps this leaves to an operator. |
...
| C2 | Model loading | ⚠️ **Partially fixed** (`f253d3d`). The repo contains no loadable synthesizer or vocoder checkpoint. `download_weights.py` used to write placeholder bytes, and `load_models()` raises on them, so the app lifespan fails. The placeholder-writing is removed and the missing-weights condition is now reported loudly instead of masked — but no real checkpoint has been provisioned, so the app still cannot run inference. | `download_weights.py` (rewritten, no longer writes placeholder bytes); `services/tts_pipeline.py:122-130, 136-144` (unchanged — already fails loudly); `main.py:79` | **Critical.** Confirmed: no `WEIGHTS_DIR` override in `.env`, network access available, no loadable checkpoint found anywhere. The server still cannot start against real inference until an operator supplies real weights (README.md § Model Weights). | Observed (executed). §4 item 1 now answered: no real weights exist outside the repo. |
...
- **C2:** ⚠️ Partial (`f253d3d`) — Placeholder-writing removed and missing-checkpoint reporting added. **Still open:** provision real TorchScript synthesizer/vocoder checkpoints (download plus checksum verification) — deferred as a separate, larger effort (see Task Status Log).
...
1. **C2 — real weights:** ✅ Answered (2026-09-15) — confirmed no `WEIGHTS_DIR` override in `backend/.env` and no real checkpoint reachable. **Still needs human input:** where to source a real, correctly-licensed SV2TTS synthesizer/vocoder checkpoint, and whether to keep the `torch.jit.load` (TorchScript) interface or rework `tts_pipeline.py` to load a raw PyTorch checkpoint (e.g. the CorentinJ Real-Time-Voice-Cloning pretrained release, which is not TorchScript-serialized and would need the actual model class vendored in). H1 and H2 still apply once real weights land.
